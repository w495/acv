...
%%cff    io:format("~p:execute start, run send_msg~n",[?MODULE]),
    EXECUTE = send_msg(Pid, {execute, Name, Version, Params, From}, From, Timeout),
%%cff    io:format("~p:execute send_msg return:~p~n", [?MODULE, EXECUTE]),
    EXECUTE.
...
%%cff    io:format("~p:send_msg start~n", [?MODULE]),
    Self = self(),
    Pid ! Msg,
    case From of
	Self ->
...
%%cff            io:format("~p:send_msg fetch_result ~n", [?MODULE]),
		    Result
	    after Timeout ->
		    {error, "message timed out"}
	    end;
	_ ->
%%cff        io:format("~p:send_msg dont wait answer, return with OK~n", [?MODULE]),
...
%%cff            io:format("~p:loop execute received (~p)... ~n", [?MODULE, self()]),
            ...
            case do_execute(State, Name, Params, Version) of
                {error, _} = Err ->
%%cff               io:format("~p:loop do_execute error: ~p~n",[?MODULE, Err]),
                    send_reply(From, Err);
%                    loop(State);
                {ok, {error, _}, _} = RERR ->
%%cff                    io:format("~p:loop do_execute error by: ~p~n",[?MODULE, RERR]);
                    error_stop_loop;
                {ok, Result, NewState}=REX ->
%%cff                    io:format("~p:loop do_execute ok: ~p~n", [?MODULE, REX]),
...
%%cff    io:format("~p:send_reply|1 start~n",[?MODULE]),
    GenSrvFrom ! {fetch_result, self(), Res};
send_reply(GenSrvFrom, Res) ->
%%cff    io:format("~p:send_reply|2 start~n",[?MODULE]),
