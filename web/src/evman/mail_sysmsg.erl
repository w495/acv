...
handle_event({evman_customer, {change, {perm, {add, {insider, Customer}}}}}, State) ->
    ?I("Customer ~p is insider ~n", [proplists:get_value("id", Customer)]),
    {ok, State};

handle_event({evman_customer, {change, {perm, {add, Perm}}}}, State) ->
    ?D("~p ~p add perm from evman_customer~n", [?HANDLERNAME, self()]),
    {ok, State};

handle_event({evman_customer, {change, {perm, {del, {insider, Customer}}}}}, State) ->
    ?I("Customer ~p is NOT insider ~n", [proplists:get_value("id", Customer)]),
    {ok, State};

handle_event({evman_customer, {change, {perm, {del, Perm}} }}, State) ->
    ?D("~p ~p del perm from evman_customer~n", [?HANDLERNAME, self()]),
    {ok, State};

handle_event({evman_customer, {change, {perm, Perm} }}, State) ->
    ?D("~p ~p has changes in perms from evman_customer~n", [?HANDLERNAME, self()]),
    {ok, State};

handle_event({evman_customer, {change, Changes }}, State) ->
    ?D("~p ~p has changes from evman_customer~n", [?HANDLERNAME, self()]),
    {ok, State};

handle_event({evman_customer, _event}, State) ->
    ?D("~p ~p has unknown event from evman_customer~n", [?HANDLERNAME, self()]),
    {ok, State};
